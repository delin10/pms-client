...
public class ContractPanel extends SwitchPanel {
	...
	public SwitchPanel initTable() {
		Func.exec(() -> {
			UIHandler.handle_contracts_table(Func.contracts(), table);
			...
			ActionListener[] listeners = new ActionListener[] { e -> {
				...
				if (selected != -1) {
						Func.exec(() -> {
							...
							m.disable(new String[] {"contract_id"});
							...
							UIHandler.showComplexe2Form(args -> {
								...
								return null;
							}, "更新住户信息", "comm?action=update&type=contract", m,params);

						});
				}
			}, e -> {
				UIHandler.handle_delete("comm?action=delete&type=contract", table, new String[] {"owner_id"} ,args->{
				...
				Func.exec(() -> {
					...
					UIHandler.showComplexe2Form(args -> {
						...
						return null;
					}, "新增楼房", "comm?action=add&type=contract", m,null);
	...
	public SwitchPanel refresh() {
		// TODO Auto-generated method stub
		UIHandler.refresh(table, "comm?action=query&type=contract");
		return null;
	}
