...
public class OwnerPanel extends SwitchPanel {
	...
	private JLabel title = new JLabel("业主列表");
	...
	public SwitchPanel initTable() {
		Func.exec(() -> {
			UIHandler.handle_owners_table(Func.owners(), table);
			...
			ActionListener[] listeners = new ActionListener[] { e -> {
				...
				if (selected != -1) {
						Func.exec(() -> {
							...
							m.disable(new String[] {"building_id","community_name","room_id","contract_id","floor_id","name","owner_id"});
							model.getValuesAt(selected).entrySet().forEach(entry->{
								String key=entry.getKey();
								m.setValue(key, entry.getValue());
							});
							String[] params= {"owner_id"};
							UIHandler.showComplexe2Form(args -> {
								...
								return null;
							}, "更新住户信息", "comm?action=update&type=owner", m,params);

						});
				}
			}, e -> {
				UIHandler.handle_delete("comm?action=delete&type=owner", table, new String[] {"owner_id"} ,args->{
				...
				Func.exec(() -> {
					...
					UIHandler.showComplexe2Form(args -> {
						...
						return null;
					}, "新增楼房", "comm?action=add&type=owner", m,null);
	...
	public SwitchPanel refresh() {
		// TODO Auto-generated method stub
		UIHandler.refresh(table, "comm?type=owner&action=query");
		return null;
	}
