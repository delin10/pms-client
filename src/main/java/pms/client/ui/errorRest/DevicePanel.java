...
public class DevicePanel extends SwitchPanel {
	...
	private JLabel title = new JLabel("设备面板");
	...
	public SwitchPanel initTable() {
		Func.exec(() -> {
			UIHandler.handle_devices_table(Func.devices(), table);
			...
			ActionListener[] listeners = new ActionListener[] { e -> {
				...
				if (selected != -1) {
					Func.exec(() -> {
						...
						UIHandler.showComplexe2Form(args -> {
							...
							return null;
						}, "更新住户信息", "comm?action=update&type=device", m, params);

					});
				}
			}, e -> {
				UIHandler.handle_delete("comm?action=delete&type=device", table, new String[] { "did" }, args -> {
				...
				Func.exec(() -> {
					...
					UIHandler.showComplexe2Form(args -> {
						...
						return null;
					}, "新增楼房", "comm?action=add&type=device", m, null);
	...
	public SwitchPanel refresh() {
		// TODO Auto-generated method stub
		UIHandler.refresh(table, "comm?action=query&type=device");;
		return null;
	}
